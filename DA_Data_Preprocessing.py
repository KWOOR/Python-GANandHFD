...
for i in range(len(finaldata)):
    ...
    a1 = pd.DataFrame(MinMaxScaler(finaldata[i].loc[390:,'Open':'ft9']))
    a2 = pd.concat([a0,a1,finaldata[i].loc[390:,'return']], axis=1 ,  sort=False)
...
totalX = total.loc[:,'Open':'ft9']# np.array(total.loc[:,'Volume':])[:1000,:]  

pca = PCA()
X=pca.fit_transform(totalX.values)
#Y=pca.fit_transform(df_new[0]['Close'].values.reshape(-1,1))

np.array(sorted(pca.explained_variance_, reverse=True))[:8].sum()/np.array(sorted(pca.explained_variance_, reverse=True))[:].sum()

totalY = total.loc[:,'return']#np.array(total['Close'])[:1000] 
