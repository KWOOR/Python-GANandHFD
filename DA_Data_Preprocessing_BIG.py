...
#os.chdir('C:\\Users\\우람\\Desktop\\UIUC\\DA')
#%%
def get_func_list():
...
for i in range(len(finaldata)):
    ...
    a1 = pd.DataFrame(MinMaxScaler(finaldata[i].iloc[390:,2:]))
    a2 = pd.concat([a0,a1], axis=1 ,  sort=False)
...
del con, a0, a1, a2,norm_finaldata, finaldata
#%%
...
np.array(sorted(pca.explained_variance_, reverse=True))[:13].sum()/np.array(sorted(pca.explained_variance_, reverse=True))[:].sum()
...
#%%
total=total.dropna()
Big_total = total.loc[:,['Dates','Ticker']]
Big_pca_total = pd.DataFrame(X[:,:13],columns=['PC1','PC2','PC3','PC4','PC5','PC6','PC7','PC8','PC9',
                             'PC10','PC11', 'PC12','PC13'])
Big_Y = pd.DataFrame(total['Close'].iloc[:,0])
Big_final = pd.concat([Big_total, Big_pca_total, Big_Y], axis=1)
    
Big_final.iloc[:int(len(Big_final)/2),:].to_csv('Big_final_data1.csv')
Big_final.iloc[int(len(Big_final)/2):,:].to_csv('Big_final_data2.csv')
