...
df1 = pd.read_csv('Big_final_data1.csv').iloc[:,1:]
df2 = pd.read_csv('Big_final_data2.csv').iloc[:,1:]
...
#%%
pred = pd.DataFrame(prediction) + pd.DataFrame(prediction).shift(1).fillna(0)
plt.plot(pred)
plt.plot(flattenY)

a = pd.DataFrame(flattenY) - pd.DataFrame(flattenY).shift(1)
b = pd.DataFrame(pred) - pd.DataFrame(flattenY).shift(1)  
#accuracy = ( sum( (a>0).values * (b>0).values ) + sum( (a<0).values * (b<0).values) )/len(flattenY)
accuracy = sum( (a.values * b.values)>0 )/len(flattenY)
print(accuracy)
