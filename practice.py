...
with tf.Session() as sess:
    ...
    for i in range(iterations):
        for j in range(0,len(trainX), pred_days):
            _, step_loss = sess.run([train, loss], feed_dict={
                                    X: np.reshape(trainX[j:j+pred_days], (1,pred_days ,input_data_column_num)) , 
                                    Y: np.reshape(trainY[int(j//pred_days)], (1,1) )})
...
#                break
    ...
    # 모델 테스트
    test_predict = []
    for i in range(0,len(testX), pred_days):
        test_predict.append(sess.run(Y_pred, feed_dict={X: np.reshape(testX[i:i+pred_days], 
                                                                      (1,pred_days,input_data_column_num))}))
...
#%%
#%%
buff = pd.read_csv('CELG.csv')['Price'] #불러온 주가로 바꿔줘라~~ 
# 3. Reverse_min_max_scaling
def reverse_standard_scaling(org_x, x):
    mu=org_x.mean()
    std = org_x.std()
    return x*std+mu

reverse_pred = reverse_standard_scaling(buff, prediction)
reverse_Y = reverse_standard_scaling(buff, flattenY)

return_list = []
s_list = [0]
for i in range(len(prediction)-1):
    
    if reverse_pred[i+1] > reverse_Y[i]+0.005:
       s = 1
    ...
    if s_list[i+1] != s_list[i]:
       ...
       r = s*np.log(reverse_Y[i+1]/reverse_Y[i])+cost*0.005
    else:
       r = s*np.log(reverse_Y[i+1]/reverse_Y[i])
       
    return_list.append(r)
    
cr = np.array(return_list)    
